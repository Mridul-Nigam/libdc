...
int clk = 0;
std::string wsIceCandidate;
std::vector<double> receivedDataX;
std::vector<double> receivedDataY;
std::vector<double> receivedDataZ;

void recordDataToCSV(const std::string& filename, std::vector<double>&receivedDataX, std::vector<double>& receivedDataY, std::vector<double>& receivedDataZX) {
...
void on_message(server* s, websocketpp::connection_hdl hdl, server::message_ptr msg) {
    if (msgRecd == 1) {
        ...
        wsIceCandidate = msg->get_payload();
        std::cout << "Received Message wsIceCAndidate: " << wsIceCandidate << std::endl;
        peer1.addRemoteCandidate(rtc::Candidate(wsIceCandidate));
        std::cout << "wsIcecandidate received" << std::endl;
        msgRecd++;
    }
    else if (msgRecd == 3) {
...
int main()
{
    ...
    channel1->onOpen([&channel1,&flag, &encoders, &scale, &done, &engaged, &master, &mx0, &my0, &mz0, &sx0, &sy0, &sz0, &slave, &tx, &ty, &tz, &fx, &fy, &fz, &time1]() {
        std::thread([&channel1, &flag,&encoders, &scale, &done, &engaged, &master, &mx0, &my0, &mz0, &sx0, &sy0, &sz0, &slave, &tx, &ty, &tz, &fx, &fy, &fz, &time1]() {
            ...
            while (!done && startDataTfr == 1) {
                ...
                if (5 > 3) {
                    ...
                    ss << i << comma << mx << comma << my << comma << mz;       // << comma << fx << comma << fy << comma << fz;
    ...
    channel1->onMessage([&channel1,&fx,&fy,&fz,&master](auto message) {
        //std::unique_lock<std::mutex>(printMutex);
        //std::thread([&channel1, &fx, &fy, &fz, &master,&message]() {
        //std::cout << std::get<std::string>(message) << std::endl;
            std::stringstream st(std::get<std::string>(message));
           
            st >> na >> comma >> fx >> comma >> fy >> comma >> fz;
            //fx = fy = fz = 0.0;
            ...
            dhdSetForce(-1*fx, -1*fy, -1*fz, master);
            ++l;
            //std::cout << "Force message " << st.str() << std::endl;
            //std::cout << st.str();
          //  }).detach();
            });
   /* if (i < 0) {
